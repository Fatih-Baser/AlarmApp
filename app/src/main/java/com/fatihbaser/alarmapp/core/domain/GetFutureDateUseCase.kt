...
class GetFutureDateUseCase {
    ...
    operator fun invoke(
        ...
        val futureDateTime = getFutureDateWithRepeatDays(
            curDateTime, hour, minute, repeatDays
        )
    ...
    private fun getFutureDateWithRepeatDays(
        curDateTime: LocalDateTime, hour: Int, minute: Int, repeatDays: Set<DayValue>
    ): LocalDateTime {
        ...
        return if (curDateTime.dayOfYear != futureDateTime.dayOfYear) {
            futureDateTime.withHour(hour).withMinute(minute).withSecond(0)
        } else {
            ...
            if ((hour >= curDateTime.hour && minute > curDateTime.minute) || hour > curDateTime.hour) {
                curDateTime.withHour(hour).withMinute(minute).withSecond(0)
            } else if (isTomorrowAvailable) {
                tomorrow.withHour(hour).withMinute(minute).withSecond(0)
            } else {
    ...
    private fun isDayOfWeekPresentInRepeatDays(
        dayOfWeek: DayOfWeek, repeatDays: Set<DayValue>
    ): Boolean {
